...
function mkl_pc_lebolide_theme_move_extra_price() {
	...
	// Defensive check: the lebolide theme references
	// MKL_PC_Extra_Price::after_add_to_cart_button, but that method no longer
	// exists on the Extra Price addon (the public surface is __construct,
	// add_choice_settings, add_db_fields, add_extra_price_to_frontend,
	// calculate_totals). Without method_exists() this throws a fatal:
	//   "TypeError: ... class MKL_PC_Extra_Price does not have a method
	//   "after_add_to_cart_button"
	// — taking the whole product page down with a 500.
	if ( $ep && method_exists( $ep, 'after_add_to_cart_button' ) ) {
